    ...
    EF: ExtensionField<F>,
{
    ...
    fn from_base(b: SymbolicExpression<F>) -> Self {
        match b {
            SymbolicExpression::Variable(v) => Self(SymbolicExpression::Variable(v.to_ext())),
    ...
    fn from_base_slice(_bs: &[SymbolicExpression<F>]) -> Self {
        todo!("from_base_slice")
    }

    fn from_base_fn<FN: FnMut(usize) -> SymbolicExpression<F>>(_f: FN) -> Self {
        todo!("from_base_fn")
    }
