...
impl<C: Config> Stark<C> for CpuStark {
    ...
    fn eval_packed_base(
        ...
        eval::<C::F, C::FE, <C::F as Field>::Packing>(vars, constraints)
    }
    ...
    fn eval_ext(
        ...
        eval::<C::FE, TrivialExtension<C::FE>, C::FE>(vars, constraints)
    }
}

fn eval<F, FE, P>(vars: AirWindow<P>, constraints: &mut ConstraintConsumer<F, FE, P>)
where
    F: Field,
